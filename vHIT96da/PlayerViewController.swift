...
//        guard let movieURL =
//            Bundle.main.url(forResource: "vhit20", withExtension: "mov") else {
//                return
//        }
        // Create an asset instance to represent the media file.
//        let avAsset = AVURLAsset(url: movieURL)
        
        loadPropertyValues(forAsset: avAsset)
    }
...
//        }
    ...
    @IBAction func togglePlay(_ sender: UIButton) {
        ...
        case .paused:
            ...
            // The player <#CMTime#>is currently paused. Begin playback.
            player.play()
        default:
            player.pause()
        }
...
//                let newTime = CMTime(seconds: Double(itemDuration), preferredTimescale: 600)
...
//                player.currentItem?.seek(to: itemDuration)
    ...
    @IBAction func playFastForward(_ sender: UIButton) {
        ...
        if player.currentItem?.currentTime() == player.currentItem?.duration {
            player.currentItem?.seek(to: CMTime.zero, completionHandler: nil)//seek(to: .zero)
        }
//        .seek(to: CMTime.zero, completionHandler: nil)
...
//        player.currentItem!.seek(to: newTime, completionHandler:nil)//toleranceBefore: .kCMTimeZero, toleranceAfter: .zero)
